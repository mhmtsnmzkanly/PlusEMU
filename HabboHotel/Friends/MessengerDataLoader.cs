...
internal class MessengerDataLoader : IMessengerDataLoader
{
    ...
    {
        using var connection = _database.Connection();
        var messages = (await connection.QueryAsync<(int, string, int)>("SELECT from_id, message, timestamp FROM messenger_offline_messages WHERE to_id = @userId", new { userId })).GroupBy(r => r.Item1).ToDictionary(r => r.Key, r => r.OrderBy(r => r.Item3).Select(g => (g.Item2, (int)(UnixTimestamp.GetNow() - g.Item3))).ToList());
