...
- Wired the minimum Nitro guide packet surface (`RequestGuideToolEvent`, `RequestGuideAssistanceEvent`, `GuideHandleHelpRequestEvent`, `GuideUserMessageEvent`, `GuideCancelHelpRequestEvent`, `GuideCloseHelpRequestEvent`, `GuideReportHelperEvent` plus the `GuideSession*` outgoing set) with Arcturus-derived packet IDs, then followed it with the first guardian packet pass (`GuardianAcceptRequestEvent`, `GuardianVoteEvent`, `GuardianNoUpdatesWantedEvent`, `GuardianNewReportReceivedComposer`, `GuardianVotingRequestedComposer`, `GuardianVotingVotesComposer`, `GuardianVotingResultComposer`), while still leaving room-invite and richer guardian lifecycle behavior for later.
...
- Hardened a few low-risk runtime paths by reusing the typed console command handler in `Program`, guarding clothing persistence against missing runtime database state, and replacing silent failure in the user process timer with explicit logging plus safer dependency checks.
- Hardened another small runtime slice by replacing exception-driven door map assignment in `Gamemap` with explicit bounds checks and by logging several previously silent failures in room-user and client-manager maintenance paths.
- Hardened a few command and parser edges too by replacing swallowed sit/lay status-add exceptions with direct guarded assignment, and by moving floorplan door-height and bot beverage parsing onto explicit bounds and `TryParse` checks.
- Hardened cache and Wired maintenance too by making the cache process overlap/disposal flow explicit and by removing empty catch blocks around direct `FurniMatchStateAndPosition` comparison checks.
- Hardened item-definition startup loading too by replacing the old console-blocking furniture-row failure path with structured logging, so bad furniture rows no longer pause headless startup.
- Tightened a couple of small nullable contracts too by making missing follow-up quests and missing highest-square items explicit in the quest and gamemap helper surfaces instead of returning `null!` from failure paths.
- Hardened a medium-sized group/navigator slice too by adding explicit null guards after group, room, and category lookup helpers before composing packet responses or reading access-control metadata.
- Finished the remaining small helper-contract cleanup by making item-name, loaded-item, and room-item lookup miss paths explicitly nullable instead of returning `null!` placeholders.
- Hardened another medium-sized query/composer slice too by adding explicit guards around room/group lookup results in moderation queries, navigator queries, room promotions, pet info composition, and login-stat favourite-group hydration.
- Continued the medium-risk lookup-contract cleanup too by making group, navigator-category, featured-room, and pending-ticket manager lookups explicitly nullable at the interface level instead of returning `null!` on miss paths.
- Continued that medium-risk contract cleanup into room lookups too by making `IRoomManager.TryGetRoom(...)` explicitly nullable on misses and tightening the surrounding group, navigator, moderation, creature, and room-settings callers to guard absent loaded rooms directly.
- Continued the moderation-side contract cleanup too by replacing the old pending-ticket getter with an explicit `TryGetTicketBySenderId(...)` flow, so support-ticket callers no longer depend on a nullable value-returning lookup that was really a `TryGet` in disguise.
- Continued the same contract cleanup into random-room lookup too by making `TryGetRandomLoadedRoom()` explicitly nullable on empty state instead of relying on a `null!` fallback behind a non-null return type.
- Continued the same medium-risk contract cleanup into room-model, quest, and chat-style lookups too by making those `TryGet...` miss paths nullable and adding explicit guards where room creation, quest reminders, and bubble/style validation consume them.
- Finished the remaining medium-risk lookup-contract sweep too by making catalog page/deal/bot, command, and room-user pet/bot `TryGet...` surfaces nullable on misses and tightening the surrounding catalog and creature flows to guard those results explicitly.
- Started the risky `RoomUserManager` lookup migration too by introducing explicit `TryGet...` helpers for square, virtual-id, habbo-id/name, and bot-name resolution, then moving the first runtime-heavy item, roller, trading, creature, and client-helper paths onto that safer surface.
- Continued that risky room-user lookup migration into packet-facing code too by moving avatar/action/user packet handlers plus the RCON motto reload path onto the new `RoomUserManager.TryGet...` helpers instead of the older null-assuming getters.
- Continued that same risky room-user lookup migration into Wired and command callers too by routing another batch of actor, triggerer, bot-target, and carried-user resolution through `RoomUserManager.TryGet...` helpers instead of the older null-assuming getters.
- Continued the same risky room-user lookup migration into interactor and service code too by moving another batch of switch, teleporter, hopper, gate, puzzle, effects, group, chat, access, moderation, and game callers onto the explicit `TryGet...` helper surface.
- Continued the same risky room-user lookup migration through another command and packet tail too by moving more room-user toggles, target-room commands, motto/effect/furniture packets, and lovelock partner resolution onto the explicit `TryGet...` helper surface.
- Continued the same risky room-user lookup migration into movement commands and `RoomUserManager` internals too by moving push/pull/mimic/pet flows plus more horse, bot-target, and square-occupancy lookups onto the explicit `TryGet...` helper surface.
- Continued the same risky room-user lookup migration through another interactor/AI tail too by moving cannon, mannequin, lovelock, bot forced-target movement, pet info/training, and kick-pets rider lookup onto the explicit `TryGet...` helper surface.
- Continued the same risky room-user lookup migration into `GameMap` occupancy checks too by routing the remaining direct square-user reads through `TryGetUserForSquare(...)` instead of the older wrapper getter.
- Tightened the same risky lookup migration at a few remaining packet and creature edges too by requiring explicit `TryGet...` success before room-user mute/respect/pet flows continue, instead of silently flowing through nullable intermediate values.
- Tightened the same lookup discipline in item runtime, chat, creature, and movement-command tails too by making a few remaining `TryGet...` calls handle failed lookups explicitly instead of relying on nullable side effects after ignored boolean results.
- Tightened the same lookup discipline in the lovelock flow too by making the remaining partner and square-user `TryGet...` calls handle failed room-user resolution explicitly instead of relying on ignored return values.
- Crossed the first structural step of that risky lookup migration too by removing the now-unused `RoomUserManager` convenience getters for square, bot-name, virtual-id, and habbo-id/name lookup once every caller had already moved to the explicit `TryGet...` surface.
- Continued that structural cleanup with a small ownership pass too: `RoomUserManager` no longer loops back through `Room.GetRoomUserManager()` for its own lookups, and `Room.UserCount` / `Room.HasUsers()` now reuse the tracked room-user count instead of rebuilding user lists.
- Continued the same structural cleanup inside `RoomUserManager`'s movement loop too by centralizing mounted-horse lookup behind a nullable-aware helper, which trims another pocket of repeated horse sync/status cleanup without changing movement behavior.
- Continued that movement-loop cleanup with two tiny helpers too: mounted move-status application and target-square occupancy writes now live behind named methods instead of repeating those low-level branches inline.
- Continued the same movement-loop cleanup with one more helper too: avatar posture and teleport/hopper transient-state reset now happen through a single named method before step application instead of repeating that cleanup inline.
