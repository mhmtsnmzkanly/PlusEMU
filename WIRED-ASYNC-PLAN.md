...
- With that done, the box type declarations have been trimmed as well, so the class signatures now reflect only the narrower actor/chat/actor-item/empty execution contracts instead of redundantly listing the broad base executable interface too.
- The queue payload shape has now been tightened too, so `WiredComponent` no longer falls back to raw `object[]` dispatch for the current trigger set and the adapter/context layer no longer needs the old parameter-based helper path in the hot flow.
- The shared execution context is now explicitly just a base type too, which matches the current model more honestly: real execution enters through chat, actor-item, actor, or empty contexts rather than ad hoc construction of the broad shell.
- Small payload parsing cleanup is continuing on top of that as well, with the selected bot/mute/move-rotate effect boxes now sharing one parser instead of each carrying their own ad hoc semicolon-splitting logic.
- The same cleanup is now reaching the state/position family too, with `MatchPosition` and the paired furni state/position conditions sharing one decoded snapshot enumeration path instead of each unpacking the saved item payloads separately.
...
- Even the last mechanical save-handler noise has been pared back now, with the leftover unused packet-read locals in the Wired boxes replaced by discard reads so the remaining surface is mostly substantive logic rather than transitional boilerplate.
- The remaining helper surface has had a final cosmetic pass too, so the new parser and dispatch helpers read more directly and carry less incidental assignment/branch noise than the first draft versions.
