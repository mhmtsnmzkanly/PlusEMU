...
- Follow-up parity fixes are in too: `RoomManager.TryGetModel(...)` once again lazy-loads custom `room_models` rows on demand, and `RoomService` doorbell entry now matches the legacy behavior by ringing whenever any room occupant is present instead of only when a rank `>= 2` user is inside.
...
- The next medium-risk query/composer pass is in too: moderation, navigator-query, room-promotion, pet-info, and login-stat loading paths now also guard missing room/group data explicitly before building responses or deriving owner/group state from lookup results.
- Another medium-risk contract pass is in too: `IGroupManager`, `INavigatorManager`, and `IModerationManager` now expose nullable miss paths for group/category/featured-room/pending-ticket lookups instead of encoding those absences behind `null!` placeholders.
- The same medium-risk contract cleanup now covers loaded-room lookups too: `IRoomManager.TryGetRoom(...)` reports missing rooms honestly with a nullable out contract, and the nearby service/query callers now guard that miss path before touching room runtime state.
- That contract cleanup now also covers moderation pending-ticket lookup: the manager exposes a real `TryGetTicketBySenderId(...)` path, and support-ticket flows no longer infer miss behavior from a nullable value-returning helper.
- The same cleanup now also covers random-room lookup: `IRoomManager.TryGetRandomLoadedRoom()` reports the empty-cache case with a nullable return instead of hiding it behind a forced non-null contract.
- The same medium-risk cleanup now also covers model/quest/style lookup helpers: room-model, quest, and chat-style `TryGet...` surfaces report misses honestly, and their nearby room/chat callers now guard those nullable results directly.
- The remaining medium-risk lookup-contract cleanup is effectively complete now: catalog page/deal/bot, command, and room-user pet/bot `TryGet...` helpers also expose honest nullable miss paths, so the leftover cleanup work is no longer broad contract hardening and instead starts crossing into riskier behavioral or structural refactor territory.
- The risky `RoomUserManager` cleanup has now started too: a real `TryGet...` lookup surface exists for square, virtual-id, habbo-id/name, and bot-name resolution, and the first runtime-heavy callers are beginning to move off the older null-assuming getter style.
- That risky `RoomUserManager` migration now reaches packet entry points as well: several avatar/action/user handlers and the RCON motto reload path now resolve room users through the newer `TryGet...` helper surface instead of the older null-assuming getters.
- That same risky `RoomUserManager` migration now covers a first Wired/command slice too: several actor, triggerer, bot-target, and carried-user resolution paths no longer depend on the older null-assuming room-user getters.
- That same risky `RoomUserManager` migration now also covers an interactor/service slice: several item interactors plus effects, group, chat, access, moderation, and game callers now resolve room users through the explicit `TryGet...` surface rather than the older getter style.
- That same risky `RoomUserManager` migration now also covers another command/packet tail: more room-user toggle commands, target-room commands, and a few avatar/effect/furniture/lovelock packets now resolve participants through the explicit `TryGet...` helper surface too.
- That same risky `RoomUserManager` migration now also reaches a first movement/internal slice: push-pull style commands and several horse, bot-target, and square-occupancy paths inside `RoomUserManager` itself now use the explicit `TryGet...` helper surface as well.
- That same risky `RoomUserManager` migration now also covers another interactor/AI slice: cannon, mannequin, lovelock, bot forced-target movement, pet info/training, and pet-kick rider lookup no longer depend on the older room-user getter style either.
- That same risky `RoomUserManager` migration now also reaches `GameMap`: the remaining direct square-occupancy reads there now go through `TryGetUserForSquare(...)` rather than the older wrapper getter.
- That same risky lookup cleanup now also tightened a few packet and creature edges: mute/respect/pet flows no longer continue past room-user resolution unless the underlying `TryGet...` actually succeeded.
- That same lookup cleanup now also tightened several remaining ignored-return sites: item runtime, chat, creature, and movement-command tails no longer depend on `TryGet...` mutating nullable locals without checking whether the lookup actually succeeded first.
- That same lookup cleanup now also reaches the lovelock flow: partner and square-user room lookups there no longer depend on ignored `TryGet...` return values either.
- That same risky lookup migration has now crossed its first structural cleanup step too: the old `RoomUserManager` convenience getters for square, bot-name, virtual-id, and habbo-id/name lookup are gone, leaving the explicit `TryGet...` surface as the real runtime contract.
- That same structural cleanup now also trims a small ownership tail: `RoomUserManager` no longer resolves itself back through `Room`, and `Room.UserCount` / `Room.HasUsers()` now read the tracked user count directly instead of materializing fresh room-user lists.
- That same structural cleanup now also reaches the movement loop internals: mounted-horse lookup is centralized behind a nullable-aware helper, reducing another cluster of repeated horse sync/status branches without changing runtime behavior.
- That same movement-loop cleanup now also names mounted move-status application and target-square occupancy writes, trimming another small cluster of inline low-level state updates out of the main walking branch.
- That same movement-loop cleanup now also centralizes avatar posture and teleport/hopper transient-state reset, trimming another inline branch out of the main step-application path without changing runtime behavior.
