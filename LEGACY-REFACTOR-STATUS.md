...
- `RoomItemHandling.SaveFurniture` now has explicit moved-item persistence helpers, `CycleRollers` is split into target-state plus item/user move helpers, `OnCycle` now isolates queue processing, and `RemoveItems` is partially decomposed, but `CheckPosItem` and the broader item service boundary are still legacy-heavy.
