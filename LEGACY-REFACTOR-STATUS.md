...
- `RoomItemHandling.SaveFurniture` now has explicit moved-item persistence helpers, and `CycleRollers` is split into target-state plus item/user move helpers, but the queued item update flow is still a dense legacy area.
