...
- `RoomItemHandling.SaveFurniture` now has explicit moved-item persistence helpers, `CycleRollers` is split into target-state plus item/user move helpers, and `OnCycle` now isolates queue processing, but the bulk item removal path is still a dense legacy area.
